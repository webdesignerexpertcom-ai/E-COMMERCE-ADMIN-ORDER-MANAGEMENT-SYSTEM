...
export default function OrderManagement() {
  ...
  const updateStatus = (status: string) => {
     ...
     setOrders(orders.map(o => o.id === activeOrderId ? { ...o, status } : o));
  ...
  const exportOrdersToCSV = () => {
    ...
    link.setAttribute("download", `ProOMS_Orders_${new Date().toISOString().split('T')[1]?.split('.')[0] || 'REPORT'}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    
    alert("Export Successful! Order report downloaded.");
  };
    ...
    <div className="space-y-6 relative">
               ...
               <div className="p-10 space-y-8">
                     ...
                     <div className="space-y-2">
                        ...
                        <div className="relative group">
                           ...
                           <input 
                              ...
                              className="w-full pl-12 pr-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:bg-white focus:border-indigo-500 transition-all outline-none"
                           />
                        </div>
                     </div>
                     <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                     <div className="space-y-2 text-bold">
                        ...
                        <input 
                           ...
                           placeholder="0.00"
                           className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:bg-white focus:border-indigo-500 transition-all outline-none"
                        />
                     </div>
                     <div className="space-y-2">
                        <label className="text-[11px] font-black uppercase text-slate-400 tracking-widest block pl-1 text-bold">Quantity Items</label>
                        <input 
                           type="number" 
                     ...
                     <div className="space-y-2">
                        ...
                        <select 
                           ...
                           className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:bg-white focus:border-indigo-500 transition-all outline-none cursor-pointer appearance-none"
                        >
                  ...
                  <div className="pt-8 border-t border-slate-100 flex items-center justify-between">
                     <div className="flex items-center gap-2 text-indigo-600">
                        <Zap className="w-5 h-5 animate-pulse font-black" />
                        <span className="text-xs font-black uppercase tracking-widest italic tracking-tighter">Fulfillment AI Active</span>
                     </div>
                     <button 
                        onClick={createOrder}
                        className="px-14 py-5 bg-indigo-600 text-white rounded-[24px] text-sm font-black uppercase tracking-widest shadow-2xl shadow-indigo-600/30 hover:bg-indigo-500 transition-all hover:scale-[1.05] active:scale-95 flex items-center gap-2"
                     >
                        Confirm & Create <CheckCircle2 className="w-5 h-5" />
                     </button>
                  </div>
               </div>
            </motion.div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          <button 
            ...
            className="flex items-center gap-2 px-6 py-3 bg-white border border-slate-200 rounded-[20px] text-sm font-black text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all shadow-sm active:scale-95"
          >
          ...
          <button 
            ...
            className="flex items-center gap-2 px-8 py-3 bg-indigo-600 text-white rounded-[20px] text-sm font-black shadow-lg shadow-indigo-600/30 hover:bg-indigo-700 transition-all hover:scale-[1.02] active:scale-[0.98]"
          >
      ...
      <div className="bg-white rounded-[48px] border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[600px] transition-all">
        <div className="p-10 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-slate-50/20">
          ...
          <div className="flex items-center gap-4">
            <button className="flex items-center gap-3 px-6 py-4 bg-white border border-slate-200 rounded-[20px] text-xs font-black uppercase tracking-widest text-slate-600 hover:bg-slate-50 transition-all shadow-sm active:scale-95">
              ...
              {orders.map((order) => {
                ...
                return (
                  <tr key={order.id} className="hover:bg-slate-50/80 transition-all group">
                        ...
                        <div className="flex flex-col">
                           ...
                           <div className="flex items-center gap-1.5 mt-2">
                              <button 
                                ...
                                className="px-2.5 py-1 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all flex items-center gap-1.5 shadow-sm"
                              >
                    ...
                    <td className="p-6">
                      <button 
                        onClick={() => { setActiveOrderId(order.id); setIsStatusPickerOpen(true); }}
                        className={cn(
                          "inline-flex items-center gap-2 px-4 py-2 rounded-[14px] text-[10px] font-black uppercase tracking-tighter border shadow-sm transition-all hover:scale-105 active:scale-95",
                    ...
                    <td className="p-6 text-center">
                      <span className="inline-flex items-center justify-center w-10 h-10 bg-white rounded-2xl text-xs font-black text-slate-800 border border-slate-100 shadow-sm">
                        {order.items}
                      </span>
                    </td>
                    <td className="p-6">
                      <span className="text-sm font-black text-slate-900 tracking-tight leading-none italic opacity-80">{order.total}</span>
                    </td>
